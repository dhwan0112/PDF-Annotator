...
function App() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      {/* Debug Overlay - Press Ctrl+Shift+D to toggle */}
      <DebugOverlay
...
function DebugOverlay({
  ...
  currentPage: number;
  currentView: string;
  tabs: Array<{ id: string; filePath: string }>;
  pdfDocument: unknown;
}) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-black/90 text-green-400 font-mono text-xs p-3 max-h-[50vh] overflow-auto z-[9999]">
      <div className="flex justify-between mb-2">
        <span className="text-yellow-400 font-bold">🐛 DEBUG (Ctrl+Shift+D to close)</span>
        <button onClick={() => setLogs([])} className="text-red-400 hover:text-red-300">Clear</button>
      </div>
      <div className="grid grid-cols-2 gap-4 mb-3">
        <div>
          <div><span className="text-gray-500">currentView:</span> {currentView}</div>
          <div><span className="text-gray-500">activeTabId:</span> {activeTabId || "null"}</div>
          <div><span className="text-gray-500">filePath:</span> {filePath ? `...${filePath.slice(-40)}` : "null"}</div>
          <div><span className="text-gray-500">currentPage:</span> {currentPage}</div>
          <div><span className="text-gray-500">pdfDocument:</span> {pdfDocument ? "loaded" : "null"}</div>
          <div><span className="text-gray-500">tabs.length:</span> {tabs.length}</div>
        </div>
        <div>
          <div className="text-gray-500 mb-1">Tabs:</div>
          {tabs.slice(0, 5).map((t) => (
            <div key={t.id} className={t.id === activeTabId ? "text-yellow-400" : ""}>
              {t.id === activeTabId ? "→ " : "  "}{t.id.slice(0, 8)}... {t.filePath.split(/[\\/]/).pop()}
            </div>
          ))}
        </div>
      </div>
